...
var totext_ToText = /** @class */ (function () {
    ...
    ToText.prototype.toString = function () {
        ...
        // @ts-ignore
        this[rrule_RRule.FREQUENCIES[this.options.freq]]();
...
function parseText(text, language) {
    ...
    function S() {
        ...
        switch (ttr.symbol) {
            case 'day(s)':
                options.freq = rrule_RRule.DAILY;
                if (ttr.nextSymbol()) {
                    AT();
                    F();
                }
            ...
            case 'weekday(s)':
                options.freq = rrule_RRule.WEEKLY;
                options.byweekday = [
                    rrule_RRule.MO,
                    rrule_RRule.TU,
                    rrule_RRule.WE,
            ...
            case 'week(s)':
                ...
                break;
            case 'hour(s)':
                options.freq = rrule_RRule.HOURLY;
                if (ttr.nextSymbol()) {
                    ON();
                    F();
                }
                break;
            case 'minute(s)':
                options.freq = rrule_RRule.MINUTELY;
                if (ttr.nextSymbol()) {
                    ON();
                    F();
                }
                break;
            case 'month(s)':
                options.freq = rrule_RRule.MONTHLY;
                if (ttr.nextSymbol()) {
                    ON();
                    F();
                }
                break;
            case 'year(s)':
                options.freq = rrule_RRule.YEARLY;
                if (ttr.nextSymbol()) {
                    ON();
                    F();
                }
            ...
            case 'sunday':
                ...
                options.byweekday = [rrule_RRule[key]];
                if (!ttr.nextSymbol())
                    return;
                ...
                while (ttr.accept('comma')) {
                    ...
                    // @ts-ignore
                    options.byweekday.push(rrule_RRule[wkd]);
                    ttr.nextSymbol();
                }
                MDAYs();
                F();
            ...
            if (nth) {
                ...
                if (wkd) {
                    ...
                    if (!options.byweekday)
                        options.byweekday = [];
                    // @ts-ignore
                    options.byweekday.push(rrule_RRule[wkd].nth(nth));
                }
                else {
...
var fromText = function (text, language) {
    if (language === void 0) { language = i18n; }
    return new rrule_RRule(parseText(text, language) || undefined);
};
var common = [
    'count',
    'until',
...
function parseOptions(options) {
    ...
    if (!(isPresent(opts.freq) && rrule_RRule.FREQUENCIES[opts.freq])) {
    ...
    if (!isPresent(opts.byweekday)) {
        ...
        for (var i = 0; i < opts.byweekday.length; i++) {
            ...
            if (!wday.n || opts.freq > rrule_RRule.MONTHLY) {
...
function buildRule(s, options) {
    ...
    if (options.forceset ||
        ...
        exrulevals.length ||
        exdatevals.length) {
        var rset_1 = new rruleset_RRuleSet(noCache);
        rset_1.dtstart(dtstart);
        rset_1.tzid(tzid || undefined);
        rrulevals.forEach(function (val) {
            rset_1.rrule(new rrule_RRule(groomRruleOptions(val, dtstart, tzid), noCache));
        });
        rdatevals.forEach(function (date) {
            rset_1.rdate(date);
        });
        exrulevals.forEach(function (val) {
            rset_1.exrule(new rrule_RRule(groomRruleOptions(val, dtstart, tzid), noCache));
        });
    ...
    var val = rrulevals[0] || {};
    return new rrule_RRule(groomRruleOptions(val, val.dtstart || options.dtstart || dtstart, val.tzid || options.tzid || tzid), noCache);
}
...
/* concated harmony reexport RRule */__webpack_require__.d(__webpack_exports__, "RRule", function() { return rrule_RRule; });
/* concated harmony reexport RRuleSet */__webpack_require__.d(__webpack_exports__, "RRuleSet", function() { return rruleset_RRuleSet; });
/*!
 * rrule.js - Library for working with recurrence rules for calendar dates.
 * https://github.com/jakubroztocil/rrule
 *
